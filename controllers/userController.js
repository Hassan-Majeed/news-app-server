  ...
  try {
    ...
    user.save(function (err, user) {
      if (err) {
        ...
        var myLink =
          'Please Click <a href="' +
          `http://localhost:${process.env.PORT}/api/users/active?userId=${user._id}` +
          '">Here</a> To Activate Your Account.';
        mailer(enteredEmail, myLink);
        res.status(201).json({
          success: true,
          msg:
            "The Activation Link has been sent to your " +
            user.email +
...
const activateUser = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      msg: "something went wrong Internal Server Error occured.",
      error: err,
    });
  }
};
